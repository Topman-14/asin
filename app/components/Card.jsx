...
export default function Card({color, shape, number, title, icon, children}) {
  return (
    <div className='p-[20px] bg-white rounded-lg w-[95%] max-w-[380px] flex-shrink-0'>
        ...
        <p>{number}</p></div>}
        {icon && <div style={{background: color}} className='w-[60px] h-[60px] flex items-center justify-center rounded-full text-white text-xl'>
        <img src={`/images/icons/${icon}`} alt='' /></div>}
